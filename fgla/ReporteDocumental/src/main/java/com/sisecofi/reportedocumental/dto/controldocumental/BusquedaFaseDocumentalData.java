package com.sisecofi.reportedocumental.dto.controldocumental;

import com.sisecofi.libreria.comunes.util.anotaciones.reportes.NameField;

import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.ToString;

/**
 * 
 * @author ayuso2104@gmail.com
 *
 */
@Data
@Builder
@ToString
@AllArgsConstructor
public class BusquedaFaseDocumentalData {

	@NameField(nameField = "id_proyecto", order = 1, nombreFront = "Id proyecto")
	private boolean idProyectoDocumentalFase;

	@NameField(nameField = "id_proyecto", order = 2, nombreFront = "Id proyecto", function = "LPAD(CAST(%valor AS TEXT), 5, '0')")
	private boolean proyectoDocumentalFase;

	@NameField(nameField = "nombre_corto", order = 3, nombreFront = "Nombre corto")
	private boolean nombreCortoDocumentalFase;

	@NameField(nameField = "'sin fase'", fieldComposite = true, order = 4, nombreFront = "Fase")
	private boolean faseDocumentalFase;

	@NameField(nameField = "'sin plantilla'", order = 5, fieldComposite = true, nombreFront = "Plantilla")
	private boolean plantillaDocumentalFase;

	@NameField(nameField = "s7.descripcion", fieldComposite = true, order = 6, nombreFront = "Descripción")
	private boolean descripcionDocumentalFase;

	@NameField(nameField = "s7.obligatorio", fieldComposite = true, order = 7, nombreFront = "Requerido")
	private boolean requeridoDocumentalFase;

	@NameField(nameField = "s7.no_aplica", fieldComposite = true, order = 8, nombreFront = "No aplica")
	private boolean noAplicaDocumentalFase;

	@NameField(nameField = "s7.estatus", fieldComposite = true, order = 9, nombreFront = "Estatus")
	private boolean estatusDocumentalFase;

	@NameField(nameField = "s7.justificacion", fieldComposite = true, order = 10, nombreFront = "Justificación")
	private boolean justificacionDocumentalFase;

	@NameField(nameField = "s7.fecha_modificacion",fieldComposite = true, order = 11, nombreFront = "Fecha última modificación", function = "TO_CHAR(%valor, 'DD/MM/YYYY')")
	private boolean fechaModificacionDocumentalFase;

	@NameField(nameField = "s7.cargado", fieldComposite = true, order = 12, nombreFront = "Cargado")
	private boolean cargadoDocumentalFase;

	@NameField(nameField = "s7.ruta", fieldComposite = true, order = 13, nombreFront = "Ruta")
	private boolean rutaDocumentalFase;

	@NameField(nameField = "s7.id", fieldComposite = true, order = 14, nombreFront = "Id")
	private boolean idArchivoDocumentalFase;
	
	@NameField(nameField = "3", fieldComposite = true, order = 15, nombreFront = "identificador")
	private boolean identificadorDocumentalFase;

	public BusquedaFaseDocumentalData() {
	    inicializarValoresFaseDocumentales();
	}

	private void inicializarValoresFaseDocumentales() {
	    setIdProyectoDocumentalFase(true);
	    setProyectoDocumentalFase(true);
	    setNombreCortoDocumentalFase(true);
	    setFaseDocumentalFase(true);
	    setPlantillaDocumentalFase(true);
	    setDescripcionDocumentalFase(true);
	    setRequeridoDocumentalFase(true);
	    setNoAplicaDocumentalFase(true);
	    setEstatusDocumentalFase(true);
	    setJustificacionDocumentalFase(true);
	    setFechaModificacionDocumentalFase(true);
	    setCargadoDocumentalFase(true);
	    setRutaDocumentalFase(true);
	    setIdArchivoDocumentalFase(true);
	    setIdentificadorDocumentalFase(true);
	}


}
