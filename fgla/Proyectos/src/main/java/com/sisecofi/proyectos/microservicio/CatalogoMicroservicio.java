...
public abstract class CatalogoMicroservicio {
	...
	public <T extends BaseCatalogoModel> List<T> obtenerInformacionCatalogo(int idCatalogo) {
		...
		if (idCatalogo <= 0) {
			throw new IllegalArgumentException("El id del catalogo no es valido");
		}

		String safeUrl = UriComponentsBuilder.fromHttpUrl(url)
				.path(PATH_INFORMACION)
				.build()
				.toUriString();

		ResponseEntity<List> responseEntity = restTemplate.exchange(safeUrl, HttpMethod.GET,
