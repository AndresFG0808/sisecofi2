...
public class SecurityConfig {
	...
	CommandLineRunner initData(UsuarioRepository userRepo, RolRepository rolRepo, PasswordEncoder encoder) {
	    return args -> {
	        ...
	        // Roles SISECOFI (formato SAT con cn=)
	        Rol sisecofiAdminSis = rolRepo.findByNombre("cn=SAT_SISECOFI_ADMIN_SIS")
	                .orElseGet(() -> {
	                    Rol r = new Rol();
	                    r.setNombre("cn=SAT_SISECOFI_ADMIN_SIS");
	                    return rolRepo.save(r);
	                });
	        
	        Rol sisecofiTodosProyectos = rolRepo.findByNombre("cn=SAT_SISECOFI_TDS_PROY")
	                .orElseGet(() -> {
	                    Rol r = new Rol();
	                    r.setNombre("cn=SAT_SISECOFI_TDS_PROY");
	                    return rolRepo.save(r);
	                });
	        
	        Rol sisecofiUsuConsulta = rolRepo.findByNombre("cn=SAT_SISECOFI_USU_CONS")
	                .orElseGet(() -> {
	                    Rol r = new Rol();
	                    r.setNombre("cn=SAT_SISECOFI_USU_CONS");
	                    return rolRepo.save(r);
	                });

	        // Usuario admin con todos los roles SISECOFI
	        if (userRepo.findByUsername("admin").isEmpty()) {
	            Usuario admin = new Usuario();
	            admin.setUsername("admin");
	            admin.setPassword(encoder.encode("admin"));
	            admin.setRoles(Set.of(adminRole, userRole, sisecofiAdminSis, sisecofiTodosProyectos, sisecofiUsuConsulta));
	            userRepo.save(admin);
	        } else {
	            // Actualizar usuario existente con roles SISECOFI
	            Usuario admin = userRepo.findByUsername("admin").get();
	            admin.setRoles(Set.of(adminRole, userRole, sisecofiAdminSis, sisecofiTodosProyectos, sisecofiUsuConsulta));
	            userRepo.save(admin);
	        }
	        
	        // Usuario básico con rol de consulta
	        if (userRepo.findByUsername("usuario").isEmpty()) {
	            Usuario user = new Usuario();
	            user.setUsername("usuario");
	            user.setPassword(encoder.encode("usuario"));
	            user.setRoles(Set.of(userRole, sisecofiUsuConsulta));
	            userRepo.save(user);
	        } else {
	            Usuario user = userRepo.findByUsername("usuario").get();
	            user.setRoles(Set.of(userRole, sisecofiUsuConsulta));
	            userRepo.save(user);
	        }
	    };
