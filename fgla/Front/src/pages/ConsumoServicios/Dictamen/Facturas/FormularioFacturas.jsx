...
import { FormatMoney, FormatFactura } from "./Components/FacturasUtilities";
...
export default function FormularioFacturas({
  ...
  const cargarXML = (event) => {
    ...
    if (!_.isEmpty(files)) {
      ...
      if (file && type === "text/xml") {
        ...
        handleShowMessage(GENERAR_FACTURA.MSG001);
      }
    }
  };
  ...
  const handleLeerXML = () => {
    const ValidacionXML = () => {
      ...
      leerXml(data).then((response) => {
        if (response?.error) {
          let mensaje = GENERAR_FACTURA.MSG001;
    ...
    if (idFactura) {
      handleShowConfirmModal(GENERAR_FACTURA.MSG009, ValidacionXML);
    } else {
      ValidacionXML();
    }
  };
  ...
  const ValidMonto = (name) => {
    ...
    if (parseFloat(montoValue) > parseFloat(totalValue)) {
      handleShowMessage(GENERAR_FACTURA.MSG012);
      return false;
    }
    return true;
  };
    ...
    <React.Fragment key={index}>
      ...
      <Row>
        ...
        <SelectArray
          ...
          options={estatusCat}
          disabled
          name={nameInput("idEstatusFactura")}
          showClasses={false}
          md={4}
        />
      </Row>
      ...
      <Row>
        <TextAreadArray
          ...
          showClasses={false}
          maxLength={!formularioActivo ? 1050 : 500}
          md={4}
        />
      </Row>
