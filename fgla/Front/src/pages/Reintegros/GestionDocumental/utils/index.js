...
function addUUIDsAndClone(data) {
  ...
  function cloneAndAddUUIDs(items, parentIdReintegro) {
    return items.map((item) => {
      // Clone the current item and add UUID
      const newItem = {
        ...item,
        idReintegro: item.idReintegro ? item.idReintegro : parentIdReintegro,
        UUID: nanoid(),
        ...
        subRows:
          item.subRows && Array.isArray(item.subRows)
            ? cloneAndAddUUIDs(
                item.subRows,
                item.idReintegro ? item.idReintegro : parentIdReintegro
              )
            : [],
      };
      return newItem;
  ...
  return cloneAndAddUUIDs(data, null);
}
