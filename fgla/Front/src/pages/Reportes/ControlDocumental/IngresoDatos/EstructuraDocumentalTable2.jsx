...
export default function EstructuraDocumentalTable2({
  ...
  useEffect(() => {
    ...
    if (idComiteProyecto && !IsEmptyArray(informacionArchivos)) {
      if (IsEmptyArray(dataTable)) {
        GetPlantillaIdComite(values.idComiteProyecto);
      }
    } else if (idPlantilla) {
      if (lastIdPlantlla !== idPlantilla) {
        ...
        setDataTable([]);
        setDocumentList([]);
        GetPlantillaIdPlantilla(values.idPlantilla);
      }
    } else {
      if (!IsEmptyArray(dataTable)) setDataTable([]);
      setDocumentList([]);
  ...
  const DeleteDocument = (row) => {
    ...
    dispatch(BorrarArchivos({ archivosEliminados: [dataToDelete] })).then(
      (response) => {
        setIsLoading(false);
        if (response.error) {
          handleShowMessage(GESTION_DOCUMENTAL.MSG0012);
        } else {
          var _documentList = [...documentList];
          let indexDoc = _documentList.findIndex((s) => id === s.idTable);
          if (indexDoc !== -1) {
    ...
    updatedDocs.forEach((document, index) => {
      ...
      if (nameFile) {
        ...
        if (_nameFile.length > 1)
          nombre = nombre + "_" + acuerdo + "." + _nameFile[1];
      }
      let _document = null;
  ...
  const CloseError = () => {
    setIsLoading(false);
    showMessage(GESTION_DOCUMENTAL.MSG006);
  };
...
function moveArchivosToSubCarpetas(
  ...
  function processCarpeta(carpeta) {
    let { idCarpetaPlantilla, archivos,descripcion:descripcionCarpeta } = carpeta;
    let _archivos = [];
...
function ValidateRequeridos(carpetas) {
  let missingData = false;

  function processCarpeta(carpeta) {
    if (carpeta.archivos) {
      const findEmpty = carpeta.archivos.find(
        (s) => {
          let { idArchivoPlantilla, obligatorio, noAplica, base64, ruta } = s;
          let missing =
            idArchivoPlantilla && obligatorio && !(noAplica || base64 || ruta);
          return missing;
    ...
    // Procesar las subcarpetas de forma recursiva
    if (carpeta.archivos) {
      for (const subCarpeta of carpeta.archivos) {
        processCarpeta(subCarpeta);
      }
    }
  }
  // Procesar todas las carpetas principales
