...
import {
  DownloadFileBase64,
  validMimeTypes,
} from "../../InformacionComites/Components/DownloadFile";
import { getMimeType } from "../../../../../functions/utils/base64.utils";
...
const getTotalDocuments = (data) => {
  ...
  allSubRows.forEach((subRow) => {
    if (!subRow?.idCarpetaPlantillaProyecto && subRow?.cargado === true) {
      loaded++;
    } else if (subRow?.cargado === false && subRow?.noAplica === false) {
      pending++;
    }
  });
