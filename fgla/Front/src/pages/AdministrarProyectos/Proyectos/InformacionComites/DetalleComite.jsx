...
export default function DetalleComite({
  ...
  const FormatMoney = (value, decimales) => {
    if (value || value === 0) {
      ...
      let formatMoney = FORMAT_MONEY.format(_value).replace("$", "");
      return formatMoney;
    }
    return "";
  };
  ...
  const [idPlantillaOriginal, setIdPlantillaOriginal] = useState(null);
  useEffect(() => {
    ...
    if (
      ...
      idsAfectacion = _.isArray(idsAfectacion) ? idsAfectacion : [];
      informacionArchivos = _.isArray(informacionArchivos)
        ? informacionArchivos
        : [];
      informacionArchivosOtrosDocumentos = _.isArray(
        informacionArchivosOtrosDocumentos
      )
        ? informacionArchivosOtrosDocumentos
        : [];
      ...
      if (infoArchivos && infoArchivos.length > 0) {
        let { carpeta } = infoArchivos[0];
      ...
      let _informacionComiteProyecto = {
        ...
        informacionArchivos: infoArchivos,
      ...
      if (asociasiones) {
        ...
        setIdPlantillaOriginal(asociasiones.idPlantillaVigente);
      }
    ...
    if (!disableEdit) {
      dispatch(UpdateComite(_values))
        .then((response) => {
          ...
          setIdPlantillaOriginal(values.idPlantilla);
        })
        .catch((error) => {
          setIsLoading(false);
          showMessage(ADMINISTRAR_INFO_COMITES.MSG007);
    ...
    >
      {({
        ...
        if (
          hasDocuments &&
          idPlantillaOriginal &&
          idPlantillaOriginal !== ValuePlantilla
        ) {
          showMessage(ADMINISTRAR_INFO_COMITES.MSG011);
          setFieldValue("idPlantilla", idPlantillaOriginal);
        }
          ...
          <>
            ...
            <Modal.Body>
              ...
              <EstructuraDocumentalTable2
                ...
                idPlantillaOriginal={idPlantillaOriginal}
              />
            </Modal.Body>
          </>
        );
